...
// Supports 10 masters and 10 slaves
// Master ID widths: CPU_Cluster_0=6, CPU_Cluster_1=6, GPU=8, Video_Encoder=4, Video_Decoder=4, DMA_Engine_0=4, DMA_Engine_1=4, PCIe_Controller=8, Master0=4, Master1=4
//==============================================================================
...
module dut_wrapper #(
    ...
    // Master 9 signals
    logic [ID_WIDTH-1:0]     m9_awid;
    logic [ADDR_WIDTH-1:0]   m9_awaddr;
    logic [7:0]              m9_awlen;
    logic [2:0]              m9_awsize;
    logic [1:0]              m9_awburst;
    logic                    m9_awlock;
    logic [3:0]              m9_awcache;
    logic [2:0]              m9_awprot;
    logic [3:0]              m9_awqos;
    logic                    m9_awvalid;
    logic                    m9_awready;
    
    logic [DATA_WIDTH-1:0]   m9_wdata;
    logic [DATA_WIDTH/8-1:0] m9_wstrb;
    logic                    m9_wlast;
    logic                    m9_wvalid;
    logic                    m9_wready;
    
    logic [ID_WIDTH-1:0]     m9_bid;
    logic [1:0]              m9_bresp;
    logic                    m9_bvalid;
    logic                    m9_bready;
    
    logic [ID_WIDTH-1:0]     m9_arid;
    logic [ADDR_WIDTH-1:0]   m9_araddr;
    logic [7:0]              m9_arlen;
    logic [2:0]              m9_arsize;
    logic [1:0]              m9_arburst;
    logic                    m9_arlock;
    logic [3:0]              m9_arcache;
    logic [2:0]              m9_arprot;
    logic [3:0]              m9_arqos;
    logic                    m9_arvalid;
    logic                    m9_arready;
    
    logic [ID_WIDTH-1:0]     m9_rid;
    logic [DATA_WIDTH-1:0]   m9_rdata;
    logic [1:0]              m9_rresp;
    logic                    m9_rlast;
    logic                    m9_rvalid;
    logic                    m9_rready;
    ...
    // Slave 9 signals
    logic [ID_WIDTH-1:0]    s9_awid;
    logic [3:0]              s9_awcache;
    logic [2:0]              s9_awprot;
    logic [3:0]              s9_awqos;
    logic [ADDR_WIDTH-1:0]   s9_awaddr;
    logic [7:0]              s9_awlen;
    logic [2:0]              s9_awsize;
    logic [1:0]              s9_awburst;
    logic                    s9_awlock;
    logic                    s9_awvalid;
    logic                    s9_awready;
    
    logic [DATA_WIDTH-1:0]   s9_wdata;
    logic [DATA_WIDTH/8-1:0] s9_wstrb;
    logic                    s9_wlast;
    logic                    s9_wvalid;
    logic                    s9_wready;
    
    logic [ID_WIDTH-1:0]    s9_bid;
    logic [1:0]              s9_bresp;
    logic                    s9_bvalid;
    logic                    s9_bready;
    
    logic [ID_WIDTH-1:0]    s9_arid;
    logic [3:0]              s9_arcache;
    logic [2:0]              s9_arprot;
    logic [3:0]              s9_arqos;
    logic [ADDR_WIDTH-1:0]   s9_araddr;
    logic [7:0]              s9_arlen;
    logic [2:0]              s9_arsize;
    logic [1:0]              s9_arburst;
    logic                    s9_arlock;
    logic                    s9_arvalid;
    logic                    s9_arready;
    
    logic [ID_WIDTH-1:0]    s9_rid;
    logic [DATA_WIDTH-1:0]   s9_rdata;
    logic [1:0]              s9_rresp;
    logic                    s9_rlast;
    logic                    s9_rvalid;
    logic                    s9_rready;
...
,
    ...
    // Slave 8 (Slave0) - Simple memory model
    always @(posedge clk) begin
        if (!rst_n) begin
            s8_awready <= 1'b0;
            s8_wready  <= 1'b0;
    ...
    // Slave 9 (Slave1) - Simple memory model
    always @(posedge clk) begin
        if (!rst_n) begin
            s9_awready <= 1'b0;
            s9_wready  <= 1'b0;
            ...
            // Write response
            if (s9_awvalid && s9_awready && s9_wvalid && s9_wready && s9_wlast) begin
                s9_bvalid <= 1'b1;
                s9_bid    <= s9_awid;
                s9_bresp  <= 2'b00; // OKAY
            end else if (s9_bready && s9_bvalid) begin
                s9_bvalid <= 1'b0;
            end
            ...
            if (s9_arvalid && s9_arready) begin
                s9_rvalid <= 1'b1;
                s9_rid    <= s9_arid;
                s9_rdata  <= {DATA_WIDTH{1'b0}}; // Return zeros
                s9_rresp  <= 2'b00; // OKAY
                s9_rlast  <= 1'b1;  // Single beat
            end else if (s9_rready && s9_rvalid) begin
                s9_rvalid <= 1'b0;
            end
        end
    end
