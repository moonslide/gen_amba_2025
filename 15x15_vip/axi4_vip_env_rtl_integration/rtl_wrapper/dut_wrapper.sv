...
module dut_wrapper #(
    ...
    // FSDB dumping for RTL wrapper internals - removed (handled by top level +all dump)
    // Top level hdl_top already dumps everything with $fsdbDumpvars(0, "hdl_top", "+all")

    // Connect signals THROUGH the RTL module (not bypassing it)
    generate
        for (i = 0; i < NUM_MASTERS; i++) begin : gen_master_connections
            // Master outputs FROM RTL MODULE back to master interface
            assign master_if[i].awready = rtl_m_awready[i];  // FROM RTL output
            assign master_if[i].wready  = rtl_m_wready[i];   // FROM RTL output
            assign master_if[i].bid     = {{(ID_WIDTH-RTL_ID_WIDTH){1'b0}}, rtl_m_bid[i]};     // FROM RTL output
            assign master_if[i].bresp   = rtl_m_bresp[i];    // FROM RTL output
            assign master_if[i].bvalid  = rtl_m_bvalid[i];   // FROM RTL output
            assign master_if[i].arready = rtl_m_arready[i];  // FROM RTL output
            assign master_if[i].rid     = {{(ID_WIDTH-RTL_ID_WIDTH){1'b0}}, rtl_m_rid[i]};     // FROM RTL output
            assign master_if[i].rdata   = rtl_m_rdata[i];    // FROM RTL output
            assign master_if[i].rresp   = rtl_m_rresp[i];    // FROM RTL output
            assign master_if[i].rlast   = rtl_m_rlast[i];    // FROM RTL output
            assign master_if[i].rvalid  = rtl_m_rvalid[i];   // FROM RTL output
        end
        
        for (i = 0; i < NUM_SLAVES; i++) begin : gen_slave_connections
            // Slave interface gets signals FROM RTL MODULE outputs
            assign slave_if[i].awid    = {{(ID_WIDTH-RTL_ID_WIDTH){1'b0}}, rtl_s_awid[i]};    // FROM RTL output
            assign slave_if[i].awaddr  = rtl_s_awaddr[i];    // FROM RTL output
            assign slave_if[i].awlen   = rtl_s_awlen[i];     // FROM RTL output
            assign slave_if[i].awsize  = rtl_s_awsize[i];    // FROM RTL output
            assign slave_if[i].awburst = rtl_s_awburst[i];   // FROM RTL output
            assign slave_if[i].awlock  = rtl_s_awlock[i];    // FROM RTL output
            assign slave_if[i].awcache = rtl_s_awcache[i];   // FROM RTL output
            assign slave_if[i].awprot  = rtl_s_awprot[i];    // FROM RTL output
            assign slave_if[i].awqos   = rtl_s_awqos[i];     // FROM RTL output
            assign slave_if[i].awregion = 4'b0;
            assign slave_if[i].awvalid = rtl_s_awvalid[i];   // FROM RTL output
            
            assign slave_if[i].wdata   = rtl_s_wdata[i];     // FROM RTL output
            assign slave_if[i].wstrb   = rtl_s_wstrb[i];     // FROM RTL output
            assign slave_if[i].wlast   = rtl_s_wlast[i];     // FROM RTL output
            assign slave_if[i].wvalid  = rtl_s_wvalid[i];    // FROM RTL output
            
            assign slave_if[i].bready  = rtl_s_bready[i];    // FROM RTL output
            
            assign slave_if[i].arid    = {{(ID_WIDTH-RTL_ID_WIDTH){1'b0}}, rtl_s_arid[i]};    // FROM RTL output
            assign slave_if[i].araddr  = rtl_s_araddr[i];    // FROM RTL output
            assign slave_if[i].arlen   = rtl_s_arlen[i];     // FROM RTL output
            assign slave_if[i].arsize  = rtl_s_arsize[i];    // FROM RTL output
            assign slave_if[i].arburst = rtl_s_arburst[i];   // FROM RTL output
            assign slave_if[i].arlock  = rtl_s_arlock[i];    // FROM RTL output
            assign slave_if[i].arcache = rtl_s_arcache[i];   // FROM RTL output
            assign slave_if[i].arprot  = rtl_s_arprot[i];    // FROM RTL output
            assign slave_if[i].arqos   = rtl_s_arqos[i];     // FROM RTL output
            assign slave_if[i].arregion = 4'b0;
            assign slave_if[i].arvalid = rtl_s_arvalid[i];   // FROM RTL output
            
            assign slave_if[i].rready  = rtl_s_rready[i];    // FROM RTL output
        end
