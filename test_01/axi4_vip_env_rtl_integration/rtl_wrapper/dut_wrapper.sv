...
// Supports 8 masters and 8 slaves
//==============================================================================
...
module dut_wrapper #(
    ...
    input  logic clk,
    input  logic rst_n,
    axi4_if.slave axi_if[8]  // Array of 8 VIP interfaces
);
...
,
    ...
    // Connect VIP interfaces to all masters
    // Master 0 connections
    assign m0_awid    = axi_if[0].awid;
    assign m0_awaddr  = axi_if[0].awaddr;
    assign m0_awlen   = axi_if[0].awlen;
    assign m0_awsize  = axi_if[0].awsize;
    assign m0_awburst = axi_if[0].awburst;
    assign m0_awlock  = axi_if[0].awlock;
    assign m0_awcache = axi_if[0].awcache;
    assign m0_awprot  = axi_if[0].awprot;
    assign m0_awqos   = 4'b0000; // Default QoS value
    assign m0_awvalid = axi_if[0].awvalid;
    assign axi_if[0].awready = m0_awready;
    
    assign m0_wdata  = axi_if[0].wdata;
    assign m0_wstrb  = axi_if[0].wstrb;
    assign m0_wlast  = axi_if[0].wlast;
    assign m0_wvalid = axi_if[0].wvalid;
    assign axi_if[0].wready = m0_wready;
    
    assign axi_if[0].bid    = m0_bid;
    assign axi_if[0].bresp  = m0_bresp;
    assign axi_if[0].bvalid = m0_bvalid;
    assign m0_bready = axi_if[0].bready;
    
    assign m0_arid    = axi_if[0].arid;
    assign m0_araddr  = axi_if[0].araddr;
    assign m0_arlen   = axi_if[0].arlen;
    assign m0_arsize  = axi_if[0].arsize;
    assign m0_arburst = axi_if[0].arburst;
    assign m0_arlock  = axi_if[0].arlock;
    assign m0_arcache = axi_if[0].arcache;
    assign m0_arprot  = axi_if[0].arprot;
    assign m0_arqos   = 4'b0000; // Default QoS value
    assign m0_arvalid = axi_if[0].arvalid;
    assign axi_if[0].arready = m0_arready;
    
    assign axi_if[0].rid    = m0_rid;
    assign axi_if[0].rdata  = m0_rdata;
    assign axi_if[0].rresp  = m0_rresp;
    assign axi_if[0].rlast  = m0_rlast;
    assign axi_if[0].rvalid = m0_rvalid;
    assign m0_rready = axi_if[0].rready;
    
    // Master 1 connections
    assign m1_awid    = axi_if[1].awid;
    assign m1_awaddr  = axi_if[1].awaddr;
    assign m1_awlen   = axi_if[1].awlen;
    assign m1_awsize  = axi_if[1].awsize;
    assign m1_awburst = axi_if[1].awburst;
    assign m1_awlock  = axi_if[1].awlock;
    assign m1_awcache = axi_if[1].awcache;
    assign m1_awprot  = axi_if[1].awprot;
    assign m1_awqos   = 4'b0000;
    assign m1_awvalid = axi_if[1].awvalid;
    assign axi_if[1].awready = m1_awready;
    
    assign m1_wdata  = axi_if[1].wdata;
    assign m1_wstrb  = axi_if[1].wstrb;
    assign m1_wlast  = axi_if[1].wlast;
    assign m1_wvalid = axi_if[1].wvalid;
    assign axi_if[1].wready = m1_wready;
    
    assign axi_if[1].bid    = m1_bid;
    assign axi_if[1].bresp  = m1_bresp;
    assign axi_if[1].bvalid = m1_bvalid;
    assign m1_bready = axi_if[1].bready;
    
    assign m1_arid    = axi_if[1].arid;
    assign m1_araddr  = axi_if[1].araddr;
    assign m1_arlen   = axi_if[1].arlen;
    assign m1_arsize  = axi_if[1].arsize;
    assign m1_arburst = axi_if[1].arburst;
    assign m1_arlock  = axi_if[1].arlock;
    assign m1_arcache = axi_if[1].arcache;
    assign m1_arprot  = axi_if[1].arprot;
    assign m1_arqos   = 4'b0000;
    assign m1_arvalid = axi_if[1].arvalid;
    assign axi_if[1].arready = m1_arready;
    
    assign axi_if[1].rid    = m1_rid;
    assign axi_if[1].rdata  = m1_rdata;
    assign axi_if[1].rresp  = m1_rresp;
    assign axi_if[1].rlast  = m1_rlast;
    assign axi_if[1].rvalid = m1_rvalid;
    assign m1_rready = axi_if[1].rready;
    
    // Master 2 connections
    assign m2_awid    = axi_if[2].awid;
    assign m2_awaddr  = axi_if[2].awaddr;
    assign m2_awlen   = axi_if[2].awlen;
    assign m2_awsize  = axi_if[2].awsize;
    assign m2_awburst = axi_if[2].awburst;
    assign m2_awlock  = axi_if[2].awlock;
    assign m2_awcache = axi_if[2].awcache;
    assign m2_awprot  = axi_if[2].awprot;
    assign m2_awqos   = 4'b0000;
    assign m2_awvalid = axi_if[2].awvalid;
    assign axi_if[2].awready = m2_awready;
    
    assign m2_wdata  = axi_if[2].wdata;
    assign m2_wstrb  = axi_if[2].wstrb;
    assign m2_wlast  = axi_if[2].wlast;
    assign m2_wvalid = axi_if[2].wvalid;
    assign axi_if[2].wready = m2_wready;
    
    assign axi_if[2].bid    = m2_bid;
    assign axi_if[2].bresp  = m2_bresp;
    assign axi_if[2].bvalid = m2_bvalid;
    assign m2_bready = axi_if[2].bready;
    
    assign m2_arid    = axi_if[2].arid;
    assign m2_araddr  = axi_if[2].araddr;
    assign m2_arlen   = axi_if[2].arlen;
    assign m2_arsize  = axi_if[2].arsize;
    assign m2_arburst = axi_if[2].arburst;
    assign m2_arlock  = axi_if[2].arlock;
    assign m2_arcache = axi_if[2].arcache;
    assign m2_arprot  = axi_if[2].arprot;
    assign m2_arqos   = 4'b0000;
    assign m2_arvalid = axi_if[2].arvalid;
    assign axi_if[2].arready = m2_arready;
    
    assign axi_if[2].rid    = m2_rid;
    assign axi_if[2].rdata  = m2_rdata;
    assign axi_if[2].rresp  = m2_rresp;
    assign axi_if[2].rlast  = m2_rlast;
    assign axi_if[2].rvalid = m2_rvalid;
    assign m2_rready = axi_if[2].rready;
    
    // Master 3 connections
    assign m3_awid    = axi_if[3].awid;
    assign m3_awaddr  = axi_if[3].awaddr;
    assign m3_awlen   = axi_if[3].awlen;
    assign m3_awsize  = axi_if[3].awsize;
    assign m3_awburst = axi_if[3].awburst;
    assign m3_awlock  = axi_if[3].awlock;
    assign m3_awcache = axi_if[3].awcache;
    assign m3_awprot  = axi_if[3].awprot;
    assign m3_awqos   = 4'b0000;
    assign m3_awvalid = axi_if[3].awvalid;
    assign axi_if[3].awready = m3_awready;
    
    assign m3_wdata  = axi_if[3].wdata;
    assign m3_wstrb  = axi_if[3].wstrb;
    assign m3_wlast  = axi_if[3].wlast;
    assign m3_wvalid = axi_if[3].wvalid;
    assign axi_if[3].wready = m3_wready;
    
    assign axi_if[3].bid    = m3_bid;
    assign axi_if[3].bresp  = m3_bresp;
    assign axi_if[3].bvalid = m3_bvalid;
    assign m3_bready = axi_if[3].bready;
    
    assign m3_arid    = axi_if[3].arid;
    assign m3_araddr  = axi_if[3].araddr;
    assign m3_arlen   = axi_if[3].arlen;
    assign m3_arsize  = axi_if[3].arsize;
    assign m3_arburst = axi_if[3].arburst;
    assign m3_arlock  = axi_if[3].arlock;
    assign m3_arcache = axi_if[3].arcache;
    assign m3_arprot  = axi_if[3].arprot;
    assign m3_arqos   = 4'b0000;
    assign m3_arvalid = axi_if[3].arvalid;
    assign axi_if[3].arready = m3_arready;
    
    assign axi_if[3].rid    = m3_rid;
    assign axi_if[3].rdata  = m3_rdata;
    assign axi_if[3].rresp  = m3_rresp;
    assign axi_if[3].rlast  = m3_rlast;
    assign axi_if[3].rvalid = m3_rvalid;
    assign m3_rready = axi_if[3].rready;
    
    // Master 4 connections
    assign m4_awid    = axi_if[4].awid;
    assign m4_awaddr  = axi_if[4].awaddr;
    assign m4_awlen   = axi_if[4].awlen;
    assign m4_awsize  = axi_if[4].awsize;
    assign m4_awburst = axi_if[4].awburst;
    assign m4_awlock  = axi_if[4].awlock;
    assign m4_awcache = axi_if[4].awcache;
    assign m4_awprot  = axi_if[4].awprot;
    assign m4_awqos   = 4'b0000;
    assign m4_awvalid = axi_if[4].awvalid;
    assign axi_if[4].awready = m4_awready;
    
    assign m4_wdata  = axi_if[4].wdata;
    assign m4_wstrb  = axi_if[4].wstrb;
    assign m4_wlast  = axi_if[4].wlast;
    assign m4_wvalid = axi_if[4].wvalid;
    assign axi_if[4].wready = m4_wready;
    
    assign axi_if[4].bid    = m4_bid;
    assign axi_if[4].bresp  = m4_bresp;
    assign axi_if[4].bvalid = m4_bvalid;
    assign m4_bready = axi_if[4].bready;
    
    assign m4_arid    = axi_if[4].arid;
    assign m4_araddr  = axi_if[4].araddr;
    assign m4_arlen   = axi_if[4].arlen;
    assign m4_arsize  = axi_if[4].arsize;
    assign m4_arburst = axi_if[4].arburst;
    assign m4_arlock  = axi_if[4].arlock;
    assign m4_arcache = axi_if[4].arcache;
    assign m4_arprot  = axi_if[4].arprot;
    assign m4_arqos   = 4'b0000;
    assign m4_arvalid = axi_if[4].arvalid;
    assign axi_if[4].arready = m4_arready;
    
    assign axi_if[4].rid    = m4_rid;
    assign axi_if[4].rdata  = m4_rdata;
    assign axi_if[4].rresp  = m4_rresp;
    assign axi_if[4].rlast  = m4_rlast;
    assign axi_if[4].rvalid = m4_rvalid;
    assign m4_rready = axi_if[4].rready;
    
    // Master 5 connections
    assign m5_awid    = axi_if[5].awid;
    assign m5_awaddr  = axi_if[5].awaddr;
    assign m5_awlen   = axi_if[5].awlen;
    assign m5_awsize  = axi_if[5].awsize;
    assign m5_awburst = axi_if[5].awburst;
    assign m5_awlock  = axi_if[5].awlock;
    assign m5_awcache = axi_if[5].awcache;
    assign m5_awprot  = axi_if[5].awprot;
    assign m5_awqos   = 4'b0000;
    assign m5_awvalid = axi_if[5].awvalid;
    assign axi_if[5].awready = m5_awready;
    
    assign m5_wdata  = axi_if[5].wdata;
    assign m5_wstrb  = axi_if[5].wstrb;
    assign m5_wlast  = axi_if[5].wlast;
    assign m5_wvalid = axi_if[5].wvalid;
    assign axi_if[5].wready = m5_wready;
    
    assign axi_if[5].bid    = m5_bid;
    assign axi_if[5].bresp  = m5_bresp;
    assign axi_if[5].bvalid = m5_bvalid;
    assign m5_bready = axi_if[5].bready;
    
    assign m5_arid    = axi_if[5].arid;
    assign m5_araddr  = axi_if[5].araddr;
    assign m5_arlen   = axi_if[5].arlen;
    assign m5_arsize  = axi_if[5].arsize;
    assign m5_arburst = axi_if[5].arburst;
    assign m5_arlock  = axi_if[5].arlock;
    assign m5_arcache = axi_if[5].arcache;
    assign m5_arprot  = axi_if[5].arprot;
    assign m5_arqos   = 4'b0000;
    assign m5_arvalid = axi_if[5].arvalid;
    assign axi_if[5].arready = m5_arready;
    
    assign axi_if[5].rid    = m5_rid;
    assign axi_if[5].rdata  = m5_rdata;
    assign axi_if[5].rresp  = m5_rresp;
    assign axi_if[5].rlast  = m5_rlast;
    assign axi_if[5].rvalid = m5_rvalid;
    assign m5_rready = axi_if[5].rready;
    
    // Master 6 connections
    assign m6_awid    = axi_if[6].awid;
    assign m6_awaddr  = axi_if[6].awaddr;
    assign m6_awlen   = axi_if[6].awlen;
    assign m6_awsize  = axi_if[6].awsize;
    assign m6_awburst = axi_if[6].awburst;
    assign m6_awlock  = axi_if[6].awlock;
    assign m6_awcache = axi_if[6].awcache;
    assign m6_awprot  = axi_if[6].awprot;
    assign m6_awqos   = 4'b0000;
    assign m6_awvalid = axi_if[6].awvalid;
    assign axi_if[6].awready = m6_awready;
    
    assign m6_wdata  = axi_if[6].wdata;
    assign m6_wstrb  = axi_if[6].wstrb;
    assign m6_wlast  = axi_if[6].wlast;
    assign m6_wvalid = axi_if[6].wvalid;
    assign axi_if[6].wready = m6_wready;
    
    assign axi_if[6].bid    = m6_bid;
    assign axi_if[6].bresp  = m6_bresp;
    assign axi_if[6].bvalid = m6_bvalid;
    assign m6_bready = axi_if[6].bready;
    
    assign m6_arid    = axi_if[6].arid;
    assign m6_araddr  = axi_if[6].araddr;
    assign m6_arlen   = axi_if[6].arlen;
    assign m6_arsize  = axi_if[6].arsize;
    assign m6_arburst = axi_if[6].arburst;
    assign m6_arlock  = axi_if[6].arlock;
    assign m6_arcache = axi_if[6].arcache;
    assign m6_arprot  = axi_if[6].arprot;
    assign m6_arqos   = 4'b0000;
    assign m6_arvalid = axi_if[6].arvalid;
    assign axi_if[6].arready = m6_arready;
    
    assign axi_if[6].rid    = m6_rid;
    assign axi_if[6].rdata  = m6_rdata;
    assign axi_if[6].rresp  = m6_rresp;
    assign axi_if[6].rlast  = m6_rlast;
    assign axi_if[6].rvalid = m6_rvalid;
    assign m6_rready = axi_if[6].rready;
    
    // Master 7 connections
    assign m7_awid    = axi_if[7].awid;
    assign m7_awaddr  = axi_if[7].awaddr;
    assign m7_awlen   = axi_if[7].awlen;
    assign m7_awsize  = axi_if[7].awsize;
    assign m7_awburst = axi_if[7].awburst;
    assign m7_awlock  = axi_if[7].awlock;
    assign m7_awcache = axi_if[7].awcache;
    assign m7_awprot  = axi_if[7].awprot;
    assign m7_awqos   = 4'b0000;
    assign m7_awvalid = axi_if[7].awvalid;
    assign axi_if[7].awready = m7_awready;
    
    assign m7_wdata  = axi_if[7].wdata;
    assign m7_wstrb  = axi_if[7].wstrb;
    assign m7_wlast  = axi_if[7].wlast;
    assign m7_wvalid = axi_if[7].wvalid;
    assign axi_if[7].wready = m7_wready;
    
    assign axi_if[7].bid    = m7_bid;
    assign axi_if[7].bresp  = m7_bresp;
    assign axi_if[7].bvalid = m7_bvalid;
    assign m7_bready = axi_if[7].bready;
    
    assign m7_arid    = axi_if[7].arid;
    assign m7_araddr  = axi_if[7].araddr;
    assign m7_arlen   = axi_if[7].arlen;
    assign m7_arsize  = axi_if[7].arsize;
    assign m7_arburst = axi_if[7].arburst;
    assign m7_arlock  = axi_if[7].arlock;
    assign m7_arcache = axi_if[7].arcache;
    assign m7_arprot  = axi_if[7].arprot;
    assign m7_arqos   = 4'b0000;
    assign m7_arvalid = axi_if[7].arvalid;
    assign axi_if[7].arready = m7_arready;
    
    assign axi_if[7].rid    = m7_rid;
    assign axi_if[7].rdata  = m7_rdata;
    assign axi_if[7].rresp  = m7_rresp;
    assign axi_if[7].rlast  = m7_rlast;
    assign axi_if[7].rvalid = m7_rvalid;
    assign m7_rready = axi_if[7].rready;
    
    // All masters are now properly connected to VIP interfaces array

    
    // Slave response logic for testing
    // Slave 0 (DDR4_Channel_0) - Simple memory model
    ...
    // Slave 1 (DDR4_Channel_1) - Simple memory model
    always @(posedge clk) begin
        if (!rst_n) begin
            s1_awready <= 1'b0;
            s1_wready  <= 1'b0;
    ...
    // Slave 2 (L3_Cache_SRAM) - Simple memory model
    always @(posedge clk) begin
        if (!rst_n) begin
            s2_awready <= 1'b0;
            s2_wready  <= 1'b0;
    ...
    // Slave 3 (Boot_ROM) - Simple memory model
    always @(posedge clk) begin
        if (!rst_n) begin
            s3_awready <= 1'b0;
            s3_wready  <= 1'b0;
    ...
    // Slave 4 (System_Registers) - Simple memory model
    always @(posedge clk) begin
        if (!rst_n) begin
            s4_awready <= 1'b0;
            s4_wready  <= 1'b0;
    ...
    // Slave 5 (PCIe_Config_Space) - Simple memory model
    always @(posedge clk) begin
        if (!rst_n) begin
            s5_awready <= 1'b0;
            s5_wready  <= 1'b0;
    ...
    // Slave 6 (Crypto_Engine) - Simple memory model
    always @(posedge clk) begin
        if (!rst_n) begin
            s6_awready <= 1'b0;
            s6_wready  <= 1'b0;
    ...
    // Slave 7 (Debug_APB_Bridge) - Simple memory model
    always @(posedge clk) begin
        if (!rst_n) begin
            s7_awready <= 1'b0;
            s7_wready  <= 1'b0;
