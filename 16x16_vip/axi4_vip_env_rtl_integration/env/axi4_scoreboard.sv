...
class axi4_scoreboard extends uvm_scoreboard;
    ...
    uvm_tlm_analysis_fifo #(axi4_master_tx) master_fifo[16];
    uvm_tlm_analysis_fifo #(axi4_slave_tx) slave_fifo[16];
    
    // Throughput tracking variables
    longint unsigned master_bytes_written[16];
    longint unsigned master_bytes_read[16];
    longint unsigned slave_bytes_written[16];
    longint unsigned slave_bytes_read[16];
    
    int unsigned master_write_count[16];
    int unsigned master_read_count[16];
    int unsigned slave_write_count[16];
    int unsigned slave_read_count[16];
    ...
    task monitor_master_transactions();
        ...
        // Create monitoring processes for all 16 masters
        for (int i = 0; i < 16; i++) begin
            fork
                automatic int idx = i;
                begin
                    forever begin
                        while (master_fifo[idx].try_get(tx)) begin
                            calculate_transaction_bytes(tx, data_bytes);
                            if (enable_latency_tracking) begin
                                track_transaction_latency(idx, tx, data_bytes);
                            end
                            if (tx.tx_type == axi4_master_tx::WRITE) begin
                                master_bytes_written[idx] += data_bytes;
                                master_write_count[idx]++;
                            end else begin
                                master_bytes_read[idx] += data_bytes;
                                master_read_count[idx]++;
                            end
                        end
                        #10ns;
                    end
    ...
    function void track_transaction_latency(int master_id, axi4_master_tx tx, int data_bytes);
        ...
        // Determine address region for path info (16 slaves with 4KB each)
        if (record.address < 32'h1000) 
            record.path_info = $sformatf("M%0d->S0 (0x%0h)", master_id, record.address);
        else if (record.address < 32'h2000) 
            record.path_info = $sformatf("M%0d->S1 (0x%0h)", master_id, record.address);
        else if (record.address < 32'h3000) 
        ...
        else if (record.address < 32'h5000) 
        ...
        else if (record.address < 32'h7000) 
        ...
        else if (record.address < 32'h9000) 
        ...
        else if (record.address < 32'hB000) 
        ...
        else if (record.address < 32'hD000) 
        ...
        else if (record.address < 32'hF000) 
