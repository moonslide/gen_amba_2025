...
class axi4_scoreboard extends uvm_scoreboard;
    ...
    uvm_tlm_analysis_fifo #(axi4_master_tx) master_fifo[17];
    uvm_tlm_analysis_fifo #(axi4_slave_tx) slave_fifo[17];
    
    // Throughput tracking variables
    longint unsigned master_bytes_written[17];
    longint unsigned master_bytes_read[17];
    longint unsigned slave_bytes_written[17];
    longint unsigned slave_bytes_read[17];
    
    int unsigned master_write_count[17];
    int unsigned master_read_count[17];
    int unsigned slave_write_count[17];
    int unsigned slave_read_count[17];
    ...
    task monitor_master_transactions();
        ...
        forever begin
            fork
                ...
                begin : master_15
                    while (master_fifo[15].try_get(tx)) begin
                        calculate_transaction_bytes(tx, data_bytes);
                        if (enable_latency_tracking) begin
                            track_transaction_latency(15, tx, data_bytes);
                        end
                        if (tx.tx_type == axi4_master_tx::WRITE) begin
                            master_bytes_written[15] += data_bytes;
                            master_write_count[15]++;
                        end else begin
                            master_bytes_read[15] += data_bytes;
                            master_read_count[15]++;
                        end
                    end
                    #10ns;
                end
                begin : master_16
                    while (master_fifo[16].try_get(tx)) begin
                        calculate_transaction_bytes(tx, data_bytes);
                        if (enable_latency_tracking) begin
                            track_transaction_latency(16, tx, data_bytes);
                        end
                        if (tx.tx_type == axi4_master_tx::WRITE) begin
                            master_bytes_written[16] += data_bytes;
                            master_write_count[16]++;
                        end else begin
                            master_bytes_read[16] += data_bytes;
                            master_read_count[16]++;
                        end
                    end
                    #10ns;
                end
    ...
    task monitor_slave_transactions();
        ...
        forever begin
            fork
                ...
                begin : slave_15
                    while (slave_fifo[15].try_get(tx)) begin
                        // Slave transaction processing (if needed)
                        if (tx.tx_type == axi4_slave_tx::WRITE) begin
                            slave_bytes_written[15] += tx.data_bytes;
                            slave_write_count[15]++;
                        end else begin
                            slave_bytes_read[15] += tx.data_bytes;
                            slave_read_count[15]++;
                        end
                    end
                    #10ns;
                ...
                begin : slave_16
                    while (slave_fifo[16].try_get(tx)) begin
                        // Slave transaction processing (if needed)
                        if (tx.tx_type == axi4_slave_tx::WRITE) begin
                            slave_bytes_written[16] += tx.data_bytes;
                            slave_write_count[16]++;
                        end else begin
                            slave_bytes_read[16] += tx.data_bytes;
                            slave_read_count[16]++;
                        end
                    end
                    #10ns;
